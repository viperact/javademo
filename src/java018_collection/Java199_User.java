...
public class Java199_User {
	...
	public static void main(String[] args) {
		...
		openAnadAllBox(fOrange);
		openAndExtedsBox(fOrange);
		// openAndsuperbox(fOrange);

		FruitBox<Seoul> fSeoul = new FruitBox<Seoul>();
		fSeoul.store(new Seoul());
		openAnadAllBox(fSeoul);
		// openAndExtedsBox(fSeoul); //Seoul은 상속을 한게 아니어서 오류발생
		openAndsuperbox(fSeoul);
	}// end main()

	public static void openAnadAllBox(FruitBox<?> param) {
		// <?>값은 Object와 같음
	...
	public static void openAndExtedsBox(FruitBox<? extends Fruit> param) {
		param.pullout().showYou();
	}// end openAndExtedsBox()
